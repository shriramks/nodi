...
export type MediaWatchActivityAnalyticsRow = MediaStatsWatchRow;
...
export type MediaTagAnalyticsRow = MediaStatsTagRow;

export type MediaRatingAnalyticsRow = MediaStatsRatingRow;
...
export async function getMediaStatsInput(
  ...
  const user = await requireUser();
  const [watchRows, tagRows, ratingRows, stateRows] = await Promise.all([
    ...
    listMediaStateAnalyticsRowsForUser(user.id, type),
  ]);

  return { watchRows, tagRows, ratingRows, stateRows };
}
...
async function listMediaStateAnalyticsRowsForUser(
  userId: string,
  type: MediaTypeFilter,
): Promise<MediaStatsStateRow[]> {
  const supabase = await createSupabaseServerClient();
  const rows: MediaStatsStateRow[] = [];
  ...
  for (let offset = 0; ; offset += mediaPageSize) {
    ...
    if (error) {
      throwDatabaseError("Failed to load media state analytics rows.", error);
    }

    const page = (data ?? []) as unknown as MediaStatsStateRow[];
    rows.push(...page);
