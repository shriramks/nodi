-- Shows with status = 'watching' (partially watched, not yet completed) were excluded from
-- the paged library RPC because the WHERE clause matched only status = 'watched'.
-- Fix: include 'watching' rows when the caller requests the watched library, and map the
-- 'watching' status to 'watched' in the output so the payload shape stays compatible with
-- the existing MovieStatus type ("watched" | "to_watch") used by the client.
...
as $$
  ...
  filtered as (
    ...
    where um.user_id = auth.uid()
      and (
        ...
        or (query_options.media_status = 'watched' and um.status = 'watching')
      )
      and (
  ...
  select
    ...
    case
      ...
      when counted.status = 'watching' then 'watched'
      else counted.status
    end as status,
